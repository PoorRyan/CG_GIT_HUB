...
uniform int u_LightingIsOff;
uniform int u_AmbientLighting;
uniform int u_SpecularLighting;
uniform int u_AmbientAndSpecularLighting;
...
void main() {
	...
	//Set up the toon shading lighting!
	if (u_ToonLighting == 1) {
	...
	vec3 result = (
		...
		if (u_LightingIsOff == 1){
			result = inColor * textureColor.rgb; 
		}
		if (u_AmbientAndSpecularLighting == 1){
